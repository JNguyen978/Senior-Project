<?php
	...
	if ($db -> connect_error) die ("Connection Failed");
	 //echo "Connected ";
		...
		// "SELECT * FROM PlayerStats p , Challenger ch , Champion c, RiotItems r WHERE ch.playerOrTeamId = p.summonerId AND c.id = p.championId AND p.item0 = r.id AND p.championId = '$id'"
		// SELECT PlayerStats.championId,PlayerStats.summonerId, Champion.id, Champion.name,Challenger.playerOrTeamId,Challenger.playerOrTeamName FROM PlayerStats,Champion,Challenger WHERE PlayerStats.championId = Champion.id AND PlayerStats.summonerId = Challenger.playerOrTeamId AND Champion.id =
	...
	mysqli_close();
			...
			// https://www.w3schools.com/tags/att_button_value.asppp
